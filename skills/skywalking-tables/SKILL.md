...
description: >
  ...
  storage families, entity ID conventions, or schema assumptions before
  generating BydbQL or assisting swctl discovery. This skill is table/domain
  guidance, not query execution.
---
...
- `swctl-query` for OAP GraphQL, trace, and profiling workflows.

## Reference Routing

- Storage family overview from SkyWalking GitHub source: read `references/generated-storage-catalog.md`.
- Metric lookup from GitHub OAL/MAL source: read `references/generated-metrics-catalog.md`.
- TopN rules from SkyWalking GitHub source: read `references/generated-topn.md`.
...
- Trace and profiling storage boundaries: read `references/traces-and-profiling.md`.
...
2. Read only the matching reference files. Prefer generated catalogs for exact source-derived names, and hand-written references for query templates and workflow boundaries.
...
Regenerate GitHub-derived catalogs with:

```bash
python3 skills/skywalking-tables/scripts/extract_skywalking_storage.py --github-repo apache/skywalking --github-ref master
```

Pass `--skywalking-root <path>` only when intentionally testing against a local checkout.
